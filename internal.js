(function(){
 ...
 window.navTool=function(id){
  ...
  id=aliases[id]||id;
  if(['uniform','best11','balance-game','boot-size','kickoff','points-sim','winrate','trip-cost'].includes(id)){location.href='/tools/'+id+'/';return false;}
  const el=$(id);
 ...
 if(['uniform','best11','balance-game','boot-size','kickoff','points-sim','winrate','trip-cost'].includes(hash)){location.replace('/tools/'+hash+'/');return;}
